...
{
    [Serializable()]
    public class FinancialIntegrity : BusinessBase<FinancialIntegrity>
    {
        #region Properties
        ...
        private FinancialIntegrity()
        { /* require use of factory methods */ }

        internal static FinancialIntegrity NewFinancialIntegrity()
        {
            return DataPortal.CreateChild<FinancialIntegrity>();
        }

        internal static FinancialIntegrity GetFinancialIntegrity(SafeDataReader dr)
        {

            return DataPortal.FetchChild<FinancialIntegrity>(dr);
        }
        ...
        public static FinancialIntegrity CreateNew()
        {
            return NewFinancialIntegrity();
        }
        ...
        protected void Child_Insert(Assistance assistance, SqlConnection cn)
        {
            if (this.IsNew)
            {
                using (SqlCommand cmd = cn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "ItsFinancialIntegritysInsert";
        ...
        protected void Child_DeleteSelf(escWorks.BusinessObjects.Assistances a)
        {
            if (this.IsDeleted)
            {
                using (SqlConnection cn = region4.Common.DataConnection.DbConnection)
                {
                    ...
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandText = "ItsFinancialIntegritysDelete";
