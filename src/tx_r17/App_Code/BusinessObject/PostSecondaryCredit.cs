...
{
    [Serializable()]
    public class PostSecondaryCredit : BusinessBase<PostSecondaryCredit>
    {
        #region Properties
        ...
        private PostSecondaryCredit()
        { /* require use of factory methods */ }

        internal static PostSecondaryCredit NewPostSecondaryCredit()
        {
            return DataPortal.CreateChild<PostSecondaryCredit>();
        }

        internal static PostSecondaryCredit GetPostSecondaryCredit(SafeDataReader dr)
        {

            return DataPortal.FetchChild<PostSecondaryCredit>(dr);
        }
        ...
        public static PostSecondaryCredit CreateNew()
        {
            return NewPostSecondaryCredit();
        }
        ...
        protected void Child_Insert(Assistance assistance, SqlConnection cn)
        {
            if (this.IsNew)
            {
                using (SqlCommand cmd = cn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "ItsPostSecondaryCreditsInsert";
        ...
        protected void Child_DeleteSelf(escWorks.BusinessObjects.Assistances a)
        {
            if (this.IsDeleted)
            {
                using (SqlConnection cn = region4.Common.DataConnection.DbConnection)
                {
                    ...
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandText = "ItsPostSecondaryCreditsDelete";
