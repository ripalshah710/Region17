...
{
    [Serializable()]
    public class StrategicPriority : BusinessBase<StrategicPriority>
    {
        #region Properties
        ...
        private StrategicPriority()
        { /* require use of factory methods */ }

        internal static StrategicPriority NewStrategicPriority()
        {
            return DataPortal.CreateChild<StrategicPriority>();
        }

        internal static StrategicPriority GetStrategicPriority(SafeDataReader dr)
        {

            return DataPortal.FetchChild<StrategicPriority>(dr);
        }
        ...
        public static StrategicPriority CreateNew()
        {
            return NewStrategicPriority();
        }
        ...
        protected void Child_Insert(Assistance assistance, SqlConnection cn)
        {
            if (this.IsNew)
            {
                using (SqlCommand cmd = cn.CreateCommand())
                {
                    ...
                    cmd.CommandText = "ItsStrategicPrioritysInsert";
        ...
        protected void Child_DeleteSelf(escWorks.BusinessObjects.Assistances a)
        {
            if (this.IsDeleted)
            {
                using (SqlConnection cn = region4.Common.DataConnection.DbConnection)
                {
                    ...
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandText = "ItsStrategicPrioritysDelete";
