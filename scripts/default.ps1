...
properties {
    ...
    $build_artifacts_dir="C:\Front-Ends\Repos\Region4\src\PrecompiledWeb\localhost20640\"
...
Write-Output $source_dir\$projectName.sln
    exec {
        & msbuild /p:OutDir=$build_artifacts_dir /t:Clean`;Rebuild /v:q /nologo /p:Configuration=$projectConfig /p:OctoPackPackageVersion=$version /p:RunOctoPack=$runOctoPack /p:OctoPackEnforceAddingFiles=true $source_dir\$projectName.sln
    }
    Write-Output 'Exit'
